...
async function handleHttpNotFound (app, ctx) {
  if (app.config.api) {
    ...
    if (ctx.path.indexOf(app.config.api) === 0) {
      ...
      if (Array.isArray(app.config.notfounds)) {
        for (const notfound of app.config.notfounds) {
          if (notfound.test && notfound.test(ctx)) {
            await notfound.handle && notfound.handle(ctx)
            return
          }
        }
      }
...
module.exports = {
  ...
  created (app) {
    ...
    if (app.config.rateLimit && app.config.rateLimit.enabled === true) {
      const limiter = ratelimit(app.config.rateLimit)
    ...
    // }
    bodyParseConfig.formidable = {
      maxFileSize: app.config.uploadQuota || 1024 * 1024 * 1024 // 设置上传文件大小最大限制，默认1G
    }
    if (app.config.bodyParser) {
      Object.assign(bodyParseConfig, app.config.bodyParser)
    }

    app.use(koaBody(bodyParseConfig))
